...
import { isNativePlatform, isElectron } from '../services/platform';
...
const PhotoLoader = () => {
  ...
  const handleSelectPhotos = async () => {
    ...
    try {
      ...
      const photos = await pickPhotosFromLibrary(GAME_CONFIG.MAX_PHOTOS_TO_PICK);
  ...
  // Bridge the macOS app menu "Select Photos…" item (Cmd+O) into the picker flow.
  useEffect(() => {
    if (!isElectron()) return;
    const handler = () => handleSelectPhotos();
    window.addEventListener('pgg:select-photos', handler);
    return () => window.removeEventListener('pgg:select-photos', handler);
  }, []);
