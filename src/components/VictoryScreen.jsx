...
const VictoryScreen = () => {
  ...
  useEffect(() => {
    if (!winner || !isElectron()) return;
    const loser = players.find((p) => p.id !== winner.id);
    const body = loser
      ? `${winner.name} ${winner.score} — ${loser.name} ${loser.score}`
      : `${winner.name} wins with ${winner.score} points!`;
    window.electronAPI.notify({ title: `${winner.name} wins!`, body });
    window.electronAPI.dock.bounce('informational');
    window.electronAPI.dock.setBadge('🏆');
    return () => {
      window.electronAPI.dock.setBadge('');
    };
  }, [winner, players]);
