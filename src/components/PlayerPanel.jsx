...
const PlayerPanel = ({ playerId, position }) => {
  ...
  // A tie-breaker can carry both scores past the winning score.
  const pointsToWin = Math.max(0, GAME_CONFIG.WINNING_SCORE - player.score);
  const scoreProgress = Math.min(100, (player.score / GAME_CONFIG.WINNING_SCORE) * 100);
  ...
  return (
    ...
    >
      ...
      <p className="text-white/50 text-xs mt-2 text-center">
        {pointsToWin > 0
          ? `${pointsToWin} point${pointsToWin === 1 ? '' : 's'} to win`
