...
const useGameStore = create(
  persist(
    (set, get) => ({
      ...
      submitGuess: (value) => {
        ...
        const determineVictory = (updatedPlayers, currentPlayerIdx) => {
          ...
          // Player 2's tie-breaker turn has ended, however it ended: it is
          // decided now. (A perfect turn that stayed under the winning score
          // used to fall through and drop the tie-breaker, so the game went on.)
        ...
        if (isCorrect) {
          const newTurnScore = state.turnScore + getPhasePoints(state.guessPhase);
          const nextPhase = getNextAnswerablePhase(state.guessPhase, gameMode, answerData);
          ...
          set({
            ...
            feedbackMessage: pointsToAdd > 0
              ? `You earned ${pointsToAdd} point${pointsToAdd > 1 ? 's' : ''} this turn.`
              : 'No points this turn.',
            correctAnswer: answerData,
            gamePhase: winner ? GAME_CONFIG.PHASES.VICTORY : GAME_CONFIG.PHASES.FEEDBACK,
            winner,
    ...
    }),
    {
      ...
      // Only the player names and the mode are kept. Photos are not saved (a
      // native photo's file URI does not outlive the session), so a game in
      // progress cannot be resumed: restoring its phase used to reopen a board
      // with "No photo available" and nothing to play.
      partialize: (state) => ({
        gameMode: state.gameMode,
      ...
      merge: (persisted, current) => ({
        ...current,
        ...(persisted?.gameMode && { gameMode: persisted.gameMode }),
        ...(Array.isArray(persisted?.players) && {
          players: current.players.map((p) => ({
            ...p,
            name: persisted.players.find((s) => s.id === p.id)?.name ?? p.name
          }))
        })
      })
    }
